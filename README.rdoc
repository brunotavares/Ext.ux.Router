...
This distribution already comes exemplified. You just have to get the code and open index.html to see it running.
All code is very well documented and decompressed.
...
2. If you followed Ext JS MVC Guide, you'll have an app.js file, which is your application entry point. Don't forget to add ux folder path to it;
...
3. Setup routes, The follow the pattern "key:value" as "url match:controler#action". Parameters are prefixed by :, e.g. users/:id/edit. It is also possible to pass parameters as plain querystrings, e.g. "help/list?viewID=3&details=1";
...
Ext.ux.Router doesn\'t handle rendering process. It only parses URI Tokens and dispatch controller\'s action. Rendering is a very specific task from each app, and would add some complexity in order to attend different scenarios.
